...
img = cv2.imread('img1.jpg') #Transformar la imagen a numpy array
...
for i in range(1, row-1):
    for j in range(1, col-1):
        kernel = [[-1,0,1],[-2,0,2],[-1,0,1]]#Kernel de la imagen
        count = 0
        for y in range(3):
            for x in range(3):  
...
print(new_img)
print(new_img[0][0])
cv2.imshow("New.jpg", new_img)#imagen con el nuevo filtro
cv2.waitKey(0)
